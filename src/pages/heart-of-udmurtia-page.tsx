...
function ArrowButton({ direction, onClick, label }: { direction: 'left' | 'right', onClick: () => void, label: string }) {
  ...
  return <button type="button" onClick={(event) => { event.stopPropagation(); onClick() }} className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-white/15 bg-black/25 text-white transition hover:border-cyan-200/70 hover:bg-cyan-300/10" aria-label={label}><Icon className="h-6 w-6" /></button>
}
...
export default function HeartOfUdmurtiaPage() {
  ...
  return <main className="mx-auto min-h-screen max-w-6xl px-5 py-8 sm:px-8 sm:py-12">
    <button type="button" onClick={() => navigate('/', { state: { scrollTo: 'projects' } })} className="inline-flex items-center gap-2 text-sm text-zinc-400 transition hover:text-cyan-100"><ArrowLeft className="h-4 w-4" />Вернуться к проектам</button>
    <motion.article initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5 }} className="mt-10">
      ...
      <div className="mt-4 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div><h1 className="max-w-4xl text-4xl font-semibold tracking-[-.05em] text-white sm:text-6xl">Сердце Удмуртии</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-zinc-300">Цифровой навигатор для туристов: интересные места, готовые маршруты, события и аудиогид в одном мобильном приложении.</p></div>
      ...
      <section className="mt-16" aria-labelledby="screencast-heading">
        ...
        <div className="relative mt-6 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-cyan-300/[.08] via-[#11131c] to-violet-500/[.12] px-4 py-6 sm:px-12 sm:py-10"><div className="absolute inset-0 opacity-40 [background-image:radial-gradient(circle_at_center,rgba(0,212,255,.16),transparent_52%)]" /><div className="relative flex flex-col items-center"><div className="flex w-full items-center justify-center gap-3 sm:gap-10"><ArrowButton direction="left" onClick={() => setScreencastSlide((current) => previous(current, screencast.length))} label="Предыдущий экран" /><button type="button" onClick={() => setLightbox({ type: 'screencast', index: screencastSlide })} className="group rounded-[2rem] border-[6px] border-zinc-700 bg-zinc-900 p-1.5 shadow-[0_20px_60px_rgba(0,0,0,.5)] transition hover:-translate-y-1 hover:border-cyan-200/70 hover:shadow-[0_20px_60px_rgba(0,212,255,.24)] focus:outline-none focus:ring-2 focus:ring-cyan-300"><span className="mx-auto mb-1.5 block h-1 w-12 rounded-full bg-zinc-600" /><img src={screencast[screencastSlide]} alt={`Экран мобильного приложения ${screencastSlide + 1}`} className="block w-[min(52vw,245px)] rounded-[1.45rem] bg-black transition duration-500 group-hover:scale-[1.015]" /></button><ArrowButton direction="right" onClick={() => setScreencastSlide((current) => next(current, screencast.length))} label="Следующий экран" /></div><div className="mt-6 flex items-center gap-3"><div className="flex max-w-48 flex-wrap justify-center gap-1.5">{screencast.map((image, index) => <button key={image} type="button" onClick={() => setScreencastSlide(index)} aria-label={`Показать экран ${index + 1}`} className={`h-1.5 rounded-full transition-all ${screencastSlide === index ? 'w-6 bg-cyan-200' : 'w-1.5 bg-white/30 hover:bg-white/70'}`} />)}</div><span className="text-xs text-zinc-400">{screencastSlide + 1} / {screencast.length}</span></div><p className="mt-3 text-sm text-zinc-400">Нажмите на экран, чтобы открыть его полностью</p></div></div>
      </section>

      <section className="mt-20" aria-labelledby="technology-heading"><div className="flex items-center gap-3"><Layers3 className="h-5 w-5 text-violet-200" /><div><p className="text-xs uppercase tracking-[.2em] text-violet-200/70">Разработка</p><h2 id="technology-heading" className="mt-1 text-2xl font-medium text-white">Стек технологий</h2></div></div><div className="mt-6 grid gap-3 md:grid-cols-3">{technologies.map((technology) => <article key={technology.label} className="rounded-2xl border border-white/10 bg-white/[.035] p-5 transition hover:-translate-y-1 hover:border-cyan-200/30 hover:bg-cyan-300/[.045]"><p className="text-xs uppercase tracking-[.2em] text-cyan-200/70">{technology.label}</p><h3 className="mt-3 text-2xl font-medium text-white">{technology.value}</h3><p className="mt-2 text-sm leading-6 text-zinc-400">{technology.detail}</p></article>)}</div></section>
