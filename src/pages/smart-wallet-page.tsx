...
function ArrowButton({ direction, onClick, label, disabled = false }: { direction: 'left' | 'right', onClick: () => void, label: string, disabled?: boolean }) {
  ...
  return <button type="button" onClick={(event) => { event.stopPropagation(); onClick() }} disabled={disabled} aria-label={label} className="grid h-12 w-12 shrink-0 place-items-center rounded-full border border-white/15 bg-black/25 text-white transition hover:border-lime-200/70 hover:bg-lime-300/10 disabled:cursor-default disabled:opacity-20"><Icon className="h-7 w-7" /></button>
}
  ...
  return <main className="mx-auto min-h-screen max-w-6xl px-5 py-8 sm:px-8 sm:py-12">
    ...
    <motion.article initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5 }} className="mt-10">
      ...
      <div className="mt-4 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div><h1 className="max-w-4xl text-4xl font-semibold tracking-[-.05em] text-white sm:text-6xl">SmartWallet — Умный кошелёк</h1><p className="mt-5 max-w-3xl text-lg leading-7 text-zinc-300">Сервис автоматически выбирает карту с максимальным кешбэком при оплате, анализируя категорию покупки, историю транзакций и геолокацию — чтобы пользователи экономили без лишних усилий.</p></div>
