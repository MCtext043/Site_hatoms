...
export default function EventPage({ title, date, subtitle, description, gallery, photos, presentation, demo }: EventPageProps) {
  ...
  return <main className="mx-auto min-h-screen max-w-6xl px-5 py-8 sm:px-8 sm:py-12">
    ...
    <motion.article initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5 }} className="mt-10">
      ...
      {description && <p className="mt-8 max-w-5xl text-base leading-8 text-zinc-300 sm:text-lg">{description}</p>}

      <section className="mt-16" aria-labelledby="gallery-heading">
        <div className="flex items-center gap-3"><Images className="h-5 w-5 text-cyan-200" /><h2 id="gallery-heading" className="text-2xl font-medium text-white">Интерфейс приложения</h2></div>
      ...
      {presentation?.length && <section className="mt-20" aria-labelledby="presentation-heading">
        ...
        {demo && <div className="mt-8 overflow-hidden rounded-2xl border border-cyan-200/20 bg-[#080b12] shadow-[0_20px_60px_rgba(0,212,255,.1)]"><div className="flex flex-col gap-1 border-b border-white/10 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs uppercase tracking-[.2em] text-cyan-200/70">В действии</p><h3 className="mt-1 text-lg font-medium text-white">Демонстрация работы прототипа</h3></div><span className="text-sm text-zinc-400">57 секунд</span></div><video controls preload="metadata" playsInline poster={presentation[0]} className="block aspect-video w-full bg-black"><source src={demo} type="video/mp4" />Ваш браузер не поддерживает воспроизведение видео.</video></div>}
      </section>}

      <a href="https://github.com/" target="_blank" rel="noreferrer" className="mt-12 inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.04] px-5 py-3 text-sm text-zinc-200 transition hover:border-cyan-200/40 hover:bg-cyan-300/[.08] hover:text-white"><GitBranch className="h-4 w-4" />GitHub — ссылка появится позже</a>
    </motion.article>
