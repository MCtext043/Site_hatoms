    ...
    <section id="home" className="relative mx-auto flex min-h-[calc(100svh-6rem)] max-w-6xl items-center px-5 pb-20 pt-24 sm:px-8">
      <div className="grid w-full items-center gap-12 lg:grid-cols-[1.15fr_.85fr]">
        <motion.div {...reveal}>
          ...
          <p className="mt-8 max-w-2xl text-pretty text-lg leading-7 text-zinc-300 sm:text-xl sm:leading-7">Превращаем идеи в работающие продукты,<br className="hidden sm:block" /> воплощая самые смелые задумки клиентов и заказчиков.</p>
    ...
    >
      ...
      <div className="project-card__content relative z-20">
        ...
        <div className="relative px-3 pb-3 pt-3">
          <h3 className="text-2xl font-medium tracking-[-.035em] text-white sm:text-[1.7rem]">{project.title}</h3>
          <p className="mt-1 max-w-2xl text-sm leading-5 text-white/90">{project.description}</p>
          {isActive && <Link to={project.caseUrl} onPointerDownCapture={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()} className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-cyan-100 transition hover:text-white">Подробнее о проекте <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></Link>}
          <div className="mt-3 flex flex-wrap gap-2.5">{project.tags.map(tag => <span key={tag} className="inline-flex items-center gap-1.5 rounded-full border border-white/[.08] bg-white/[.055] px-3 py-2.5 text-sm font-medium tracking-wide text-zinc-100"><Cpu className="h-3.5 w-3.5 sm:hidden" />{tag}</span>)}</div>
        </div>
      </div>
    </motion.article>
  )
...
export function Projects() {
  ...
  return (
    <section id="projects" className="relative overflow-visible px-5 py-28 sm:px-8 sm:py-32">
      <motion.div {...reveal} className="mx-auto max-w-6xl"><SectionHeading eyebrow="Проекты команды" title={<>Наши <span className="text-gradient">проекты</span></>} description="Идеи, которым мы придали форму, логику и заметный результат." descriptionClassName="mt-3 text-white" /></motion.div>
...
function LegacyTechStack() {
  const providers = [
    ...
    { title: 'Т-Образование', logo: '/logos/education/t-education.webp', courses: ['Алгоритмы и структуры данных', 'Android для начинающих'] },
  ]
  ...
  return (
    <section id="stack" className="mx-auto max-w-6xl px-5 py-28 sm:px-8">
      <motion.div {...reveal}>
        <SectionHeading
          eyebrow="Техническая экспертиза"
          title={<>Наше <span className="text-gradient">программное обеспечение</span></>}
          description="Ключевые технологии и программное обеспечение по направлениям"
          descriptionClassName="mt-1 text-white"
        />
      </motion.div>

      <motion.div {...reveal} className="mt-12">
        <h3 className="mb-5 text-2xl font-medium tracking-[-.02em] text-white sm:text-3xl">Основные технологии</h3>
      ...
      <motion.div {...reveal} className="mt-14">
        ...
        <motion.div key={activeCategory.title} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .25 }} className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {activeCategory.technologies.map(tech => (
            <div key={tech} className="group relative flex min-h-24 items-center overflow-hidden rounded-xl border border-white/10 bg-white/[.025] px-4 py-4 text-base text-zinc-300 transition hover:scale-[1.025] hover:border-cyan-200/30 hover:bg-cyan-300/[.05] hover:text-white">
              ...
              <span className="pointer-events-none absolute inset-0 grid place-items-center translate-y-3 opacity-0 text-4xl text-cyan-100 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100 group-hover:drop-shadow-[0_0_12px_rgba(0,212,255,.65)]"><TechnologyLogo name={tech} /></span>
            </div>
          ))}
        </motion.div>
      </motion.div>
...
export function Events() {
  return <section id="events" className="mx-auto max-w-6xl px-5 py-28 sm:px-8"><motion.div {...reveal}><SectionHeading eyebrow="Опыт команды" title={<>Мероприятия и <span className="text-gradient">события</span></>} /></motion.div><div className="mt-12 border-l border-white/10 pl-6 sm:pl-10">{events.map((event, i) => <motion.article key={event.slug} {...reveal} transition={{ duration: .6, delay: i * .1 }} className="relative mb-10 last:mb-0"><span className="absolute -left-[31px] top-1 h-3 w-3 rounded-full border-2 border-[#05060a] bg-cyan-300 shadow-[0_0_14px_#00d4ff] sm:-left-[47px]" /><p className="font-mono text-xs font-normal tracking-wider text-cyan-200/70">{event.date}</p><Link to={`/events/${event.slug}`} className="group mt-3 block rounded-2xl border border-white/10 bg-white/[.025] p-5 transition hover:border-violet-300/30 hover:bg-white/[.04] focus:outline-none focus:ring-2 focus:ring-cyan-300/70"><h3 className="text-lg font-medium text-white">{event.title}</h3>{(event.summary || event.subtitle) && <p className="mt-0.5 max-w-2xl text-sm font-normal leading-6 text-zinc-300">{event.summary || event.subtitle}</p>}<span className="mt-4 inline-flex items-center gap-2 text-sm text-white">Подробнее <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></span></Link></motion.article>)}</div></section>
}
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-labelledby="request-title" onMouseDown={onClose}>
      ...
      <form onSubmit={submit} className="mt-6 rounded-[1.75rem] border border-white/10 bg-white/[.025] p-5 sm:p-7">
        ...
        <label className="mt-5 flex cursor-pointer items-start gap-3 text-sm leading-5 text-zinc-400"><span className="mt-0.5 grid h-4 w-4 shrink-0"><input name="personalDataConsent" type="checkbox" required className="peer sr-only" /><span aria-hidden="true" className="grid h-4 w-4 place-items-center rounded border border-white/10 bg-black/20 text-[10px] leading-none text-transparent transition peer-checked:border-zinc-400 peer-checked:bg-black peer-checked:text-white peer-focus-visible:ring-2 peer-focus-visible:ring-cyan-200/60">✓</span></span><span>Согласен на обработку указанных контактов исключительно для ответа на заявку.</span></label>
    ...
    <footer id="contacts" className="border-t border-white/10 px-5 py-14 sm:px-8">
      <div className="mx-auto flex max-w-6xl flex-col gap-10 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex flex-col items-start gap-3 md:items-end">
          <button type="button" onClick={onOpenRequest} className="group inline-flex items-center gap-3 rounded-full bg-white px-5 py-3 text-sm font-semibold text-zinc-950 shadow-[0_0_28px_rgba(255,255,255,.22)] transition hover:-translate-y-0.5 hover:bg-zinc-200 hover:shadow-[0_0_38px_rgba(255,255,255,.32)]"><span>Оставить заявку</span><ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></button>
